#! /usr/bin/env python
"""A helper program to drive Jaco2 arm to a declared position."""
...
def getcurrentJointCommand(prefix_):
    ...
    rospy.Subscriber(topic_address, kinova_msgs.msg.JointAngles,
                     setcurrentJointCommand)
...
def argumentParser(argument):
    ...
    parser.add_argument('joint_value', nargs='*', type=float,
                        help='joint values, length equals to number of joints.')
...
def unitParser(unit, joint_value, relative_):
    ...
    if unit == 'degree':
        ...
        # get absolute value
        if relative_:
            joint_degree_absolute_ = [
                joint_degree_command[i] + currentJointCommand[i] for i in range(0, len(joint_value))]
        else:
...
def verboseParser(verbose, joint_degree):
    ...
    if verbose:
        ...
        print(', '.join('joint{:1.0f} {:3.1f}'.format(
            k[0] + 1, k[1]) for k in enumerate(joint_degree)))
        print('Joint values sent in radian are: ')
        print(', '.join('joint{:1.0f} {:1.3f}'.format(
            k[0]+1, k[1]) for k in enumerate(joint_radian)))
...
if __name__ == '__main__':
    ...
    if len(args.joint_value) != arm_joint_number:
        print('Number of input values {} is not equal to number of joints {}. Please run help to check number of joints with different robot type.'.format(
            len(args.joint_value), arm_joint_number))
        sys.exit(0)

    # get Current finger position if relative position
    getcurrentJointCommand(prefix)
    joint_degree, joint_radian = unitParser(
        args.unit, args.joint_value, args.relative)

    positions = [0]*arm_joint_number
    try:
