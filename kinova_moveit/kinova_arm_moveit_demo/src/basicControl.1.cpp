...
moveit::planning_interface::MoveGroup::Plan evaluate_plan(
    moveit::planning_interface::MoveGroup& group)
{
    ...
    while (replan == true && ros::ok()) {
        ...
        {
            std::cout << "Exit since plan failed until reach maximum attemp: " << count
                      << std::endl;
            replan = false;
            break;
        }
...
bool gripper_action(bool robot_connected_, moveit::planning_interface::MoveGroup* gripper_group_,
...
marker_visualize::marker_visualize(ros::NodeHandle& nh)
    : n_(nh)
{
...
int main(int argc, char** argv)
{
    ...
    // can moveit handle a manual defined end-effector frame instead of frames
    // parsed from urdf
    ...
    ROS_INFO_STREAM("Input any char and Enter to move to predifined start pose "
                    "(0.1, -0.5, 0.5) ...");
    std::cin >> pause_;
    ...
    ROS_INFO_STREAM(
        "Last point of Plan:" << my_plan.trajectory_.joint_trajectory.points[plan_point_size - 1]);
    ...
    for (int i = 0; i < plan_point_size; i++) {
        ...
        for (int j = 0; j < 6; j++) {
            ...
            if (j == 5) {
                ...
                ROS_INFO(
                    "Planed Point:%f %f %f", pose_.position.x, pose_.position.y, pose_.position.z);
            }
        }
    }
    ...
    for (int i = 0; i < forKJointValue.size(); i++) {
        ...
        if (i == forKJointValue.size() - 1) std::cout << std::endl;
    }
    ...
    ROS_INFO_NAMED(
        "tutorial", "Visualizing plan (Cartesian path) (%.2f%% acheived)", fraction * 100.0);
    ...
    ROS_INFO_STREAM(
        "Last point of Plan:" << trajectory.joint_trajectory.points[plan_point_size - 1]);
