...
Parser::Parser()
{
	...
	double d4 = 0.0743;	// First wrist length
	double d5 = 0.0743;	// Second wrist length
	double d6 = 0.1687;	// Wrist to center of the hand
	double e2 = 0.0098;	// Joint 3-4 lateral offset

	double aa = ((11.0*Pi) / 72.0);
	double ca = (cos(aa));
	double sa = (sin(aa));
	double c2a = (cos(2 * aa));
	double s2a = (sin(2 * aa));
	...
	dh << Pi / 2, 0, 0, d1, Pi, d2, 0, 0, Pi / 2, 0, 0, -e2, 2 * aa, 0, 0, -d4b, 2 * aa, 0, 0, -d5b, Pi, 0, 0, -d6b;
