...
class notice_pub_sub
{
  ...
  public:
	...
	void notice_msgCallback(const id_data_msgs::ID_Data::ConstPtr &notice_msg)
	{
		...
		if (notice_message.id == 4 && notice_message.data[0] == 1 && suck_obj_list.count(notice_message.data[1]) == 0)
		{
			...
			if (fabs(x) < 0.5 && y < -0.3)
			{
				...
				// TODO
				if (z < DESK_HEIGHT_MIDDLE)
					target_from_kinect.table_type = LOW;
				if (z > DESK_HEIGHT_MIDDLE && z < DESK_HEIGHT_HIGH)
					target_from_kinect.table_type = MIDDLE;
				if (z > DESK_HEIGHT_HIGH)
					target_from_kinect.table_type = HIGH;
...
void poseInit()
{
	...
	temp.orientation = tf::createQuaternionMsgFromRollPitchYaw(1.4, -0.01, -0.1); // grasp orientation
