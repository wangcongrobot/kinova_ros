...
void notice_pub_sub::notice_pub_sub_pulisher(id_data_msgs::ID_Data id_data) { notice_publisher.publish(id_data); }
...
void poseInit()
{
	grasp_pose.orientation = tf::createQuaternionMsgFromRollPitchYaw(1.57, -1.0, 0.0); // grasp orientation
...
void confirmToAct(const geometry_msgs::Pose &start, const geometry_msgs::Pose &goal, const string &str);
void confirmToAct(const geometry_msgs::Pose &goal, const string &str);
void handleCollisionObj(build_workScene &buildWorkScene);
void moveLineTarget(const geometry_msgs::Pose &start, const geometry_msgs::Pose &goal);
ErrorCode hand_MsgConform_ActFinishedWait(id_data_msgs::ID_Data *notice_data_test, bool *msg_rec_flag,
                                          bool *finished_flag, notice_pub_sub *notice_test, string task);
...
int main(int argc, char **argv)
{
	...
	planning_scene_interface.addCollisionObjects(buildWorkScene.collision_objects); // add collision objects into world
	...
	while (ros::ok()) {
			...
			if (use_gripper_flag)
			{
				...
				if (hand_current_mode != "grasp")
				{
					...
					ErrorCode err = hand_MsgConform_ActFinishedWait(&notice_data, &hand_msg_rec_flag,
					                                                &hand_act_finished_flag, &notice_test, "SWITCH");
					error_deal(err);
		...
		if (arm_release_obj_flag)
		{
			...
			ErrorCode err = hand_MsgConform_ActFinishedWait(&notice_data, &hand_msg_rec_flag, &hand_act_finished_flag,
			                                                &notice_test, "RELEASE");
			error_deal(err);
                                          ...
                                          bool *finished_flag, notice_pub_sub *notice_test, string task)
{
	...
	while (ros::ok()) {
		...
		{
			if (task == "FETCH")
				ROS_INFO("Waiting for hand to grasp/suck...");
			if (task == "RELEASE")
				ROS_INFO("Waiting for hand to open/release...");
			if (task == "SWITCH")
				ROS_INFO("Waiting for hand to switch mode...");
		}
		notice_test->notice_sub_spinner(1);
		loop_rate.sleep();
	}
...
void moveToTarget(const geometry_msgs::Pose &target)
{
	...
	for (int i = 0; i < loops; i++) {
		...
		if (success)
		{
			...
			if (plan_steps < 25)
			{
				ROS_INFO_STREAM("Try " << i << ": plan found in " << my_plan.planning_time_ << " seconds with "
				                       << plan_steps << " steps");
				plan_valid = true;
				break;
			}
...
void moveLineTarget(const geometry_msgs::Pose &start, const geometry_msgs::Pose &goal)
{
	...
	if (plan_steps < 30)
	{
		ROS_INFO_STREAM("Plan found in " << cartesian_plan.planning_time_ << " seconds with " << plan_steps
		                                 << " steps");
...
void confirmToAct(const geometry_msgs::Pose &start, const geometry_msgs::Pose &goal, const string &str = "NULL")
{
	cout << "\n"
	     << "=================MOVE TO " + str + "=================="
	     << "\n";
