...
void notice_pub_sub::notice_msgCallback(
  ...
  if (notice_message.id == 2 &&
      notice_message.data[0] == 14) // msg received flag
  {
    nav_msg_rec_flag = true;
  }
  if (notice_message.id == 2 &&
      notice_message.data[0] == 15) // nav finished flag
  {
    nav_finished_flag = true;
  }
  if (notice_message.id == 2 &&
      notice_message.data[0] == 16) // nav finished flag
  {
    set_ontime++;
  ...
  if (notice_message.id == 3 &&
      notice_message.data[0] == 14) // kinect msg received flag
  {
    kinect_msg_rec_flag = true;
  }
  if (notice_message.id == 3 &&
      notice_message.data[0] == 15) // kinect scan finished flag
  {
    kinect_scan_finished_flag = true;
  }
  if (notice_message.id == 3 &&
      notice_message.data[0] == 13) // kinect scan failed
  {
    ...
    ROS_WARN(
        "Received missing current column flag.Save data No.%d,Row:%d,Col%d",
        obj_cnt, row[obj_cnt - 1], column[obj_cnt - 1]);
    kinect_scan_finished_flag = true;
  }
  // 3,arm control section
  if (notice_message.id == 4 &&
      notice_message.data[0] == 14) // arm control msg received flag
  {
    arm_msg_rec_flag = true;
  }
  if (notice_message.id == 4 &&
      notice_message.data[0] == 15) // arm fetch finished flag
  {
...
void error_deal(int error_nu) {
  ...
  default:
    ROS::ROS_WARN("No feedback from execution stage!"); // Alvin
    break;
  }
}
...
int main(int argc, char **argv) {
  ...
  cout << "Object JOY number:" << obj_num << "\t"
       << "JOY Object position:" << endl;
  for (int i = 0; i < obj_num; i++)
    cout << "Column:" << column[i] << " "
         << "Row:" << row[i] << endl;
  ...
  while (ros::ok()) {
    for (loop_sys_cnt = 0; loop_sys_cnt < obj_num; loop_sys_cnt++) {
      ...
      error_no = main_MsgConform_ActFinishedWait(
          &notice_data, &nav_msg_rec_flag, &nav_finished_flag, &notice_test);
      error_deal(error_no);
      ...
      error_no = main_MsgConform_ActFinishedWait(
          &notice_data, &kinect_msg_rec_flag, &kinect_scan_finished_flag,
          &notice_test);
      error_deal(error_no);
      ...
      notice_data.data[0] = 1;
      error_no =
          main_MsgConform_ActFinishedWait(&notice_data, &arm_msg_rec_flag,
                                          &arm_act_finished_flag, &notice_test);
      error_deal(error_no);
      // arm keep pose
