...
bool gripper_action(bool robot_connected_,
                    ...
                    actionlib::SimpleActionClient<
                        kinova_msgs::SetFingersPositionAction> *finger_client_,
                    double finger_turn) {
...
void marker_visualize::add_markerPoints(int marker_id,
...
void marker_visualize::add_markerLinestrip(int marker_id,
                                           geometry_msgs::Pose &pose_) {
...
int main(int argc, char **argv) {
  ...
  for (int i = 0; i < joint_name_size; i++) {
    ROS_INFO("joint name:%s",
             my_plan.trajectory_.joint_trajectory.joint_names[i].c_str());
  }
  ROS_INFO_STREAM(
      "Last point of Plan:"
      << my_plan.trajectory_.joint_trajectory.points[plan_point_size - 1]);
  ...
  for (int i = 0; i < plan_point_size; i++) {
    ...
    for (int j = 0; j < 6; j++) {
      joint_value[j] =
          my_plan.trajectory_.joint_trajectory.points[i].positions[j];
      if (j == 5) {
        ...
        ROS_INFO("Planed Point:%f %f %f", pose_.position.x, pose_.position.y,
                 pose_.position.z);
      }
    }
  }
  ...
  double fraction = group_->computeCartesianPath(waypoints, eef_step,
  ...
  ROS_INFO_STREAM("Last point of Plan:"
                  << trajectory.joint_trajectory.points[plan_point_size - 1]);
