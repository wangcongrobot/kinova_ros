...
void notice_pub_sub::notice_msgCallback(const id_data_msgs::ID_Data::ConstPtr& notice_msg)
{
    ...
    if (notice_message.id == 3 && notice_message.data[0] == 13) // kinect scan failed
    {
        ...
        ROS_WARN("Received missing current column flag.Save data No.%d,Row:%d,Col%d", obj_cnt,
            row[obj_cnt - 1], column[obj_cnt - 1]);
        kinect_scan_finished_flag = true;
    }
...
int main_MsgConform_ActFinishedWait(id_data_msgs::ID_Data* notice_data_test, bool* msg_rec_flag,
...
int main(int argc, char** argv)
{
    ...
    while (ros::ok()) {
        for (loop_sys_cnt = 0; loop_sys_cnt < obj_num; loop_sys_cnt++) {
            ...
            error_no = main_MsgConform_ActFinishedWait(
                &notice_data, &kinect_msg_rec_flag, &kinect_scan_finished_flag, &notice_test);
            error_deal(error_no);
            ...
            notice_data.data[0] = 1;
            error_no = main_MsgConform_ActFinishedWait(
                &notice_data, &arm_msg_rec_flag, &arm_act_finished_flag, &notice_test);
            error_deal(error_no);
            // arm keep pose
