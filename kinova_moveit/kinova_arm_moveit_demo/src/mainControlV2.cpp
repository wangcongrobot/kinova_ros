...
void notice_pub_sub::notice_msgCallback(const id_data_msgs::ID_Data::ConstPtr &notice_msg)
{
	...
	else if (notice_message.id == 4 && notice_message.data[0] == 1 && notice_message.data[1] == 8)
	{
...
void poseInit()
{
	...
	suck_pose.orientation = tf::createQuaternionMsgFromRollPitchYaw(1.57, -1.2, 0.0); // grasp orientation
	// rest pose
...
int confirmToAct(const geometry_msgs::Pose &start, const geometry_msgs::Pose &goal, const string &str = "NULL")
{
	cout << "\n"
	     << "=================MOVE TO " + str + "=================="
	     << "\n";
...
int hand_MsgConform_ActFinishedWait(id_data_msgs::ID_Data *notice_data_test, bool *msg_rec_flag, bool *finished_flag,
...
void moveToTarget(const geometry_msgs::Pose &target)
{
	...
	ROS_INFO_STREAM("Planning to move " << group.getEndEffectorLink() << " to a target pose expressed in "
