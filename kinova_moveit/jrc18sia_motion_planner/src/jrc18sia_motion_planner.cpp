...
JRCMotionPlanner::JRCMotionPlanner(ros::NodeHandle &nh)
    : nh_(nh), group_name_("arm"), planning_id_("RRTConnectkConfigDefault"), planning_attempts_(1),
      ...
      jump_threshold_(2.0), trajectory_velocity_scaling_(TRAJECTORY_VELOCITY_SCALING), // trajectory processing
      max_plan_steps_(MAX_PLAN_STEP), max_cartesion_plan_steps_(MAX_CART_PLAN_STEP),
      ...
      kinova_driver_tool_pose_topic_("j2n6s300_driver/out/tool_pose"), joint_states_topic_("joint_states")
{
...
std::vector<double> JRCMotionPlanner::getCurrentJointState()
{
	...
	sensor_msgs::JointStateConstPtr joint_state =
	    ros::topic::waitForMessage<sensor_msgs::JointState>(joint_states_topic_, nh_, ros::Duration(1.0));
	if (!joint_state)
